...
const REF={w:1448,h:1086},worldLayers=[...document.querySelectorAll("[data-scene-world]")],objectLayers=[...document.querySelectorAll("[data-scene-object]")],validGateStates=new Set(["normal","unstable","event"]),params=new URLSearchParams(location.search),config=document.currentScript?.dataset||{},defaultDepth=location.pathname.includes("tarot-breaker-game-preview")?"./star-country-gate-garden-depth.json":"./assets/maps/star-country-gate-garden-depth.json",DEPTH_URL=config.depthUrl||defaultDepth,layout=window.TarotSceneLayout;
let lastCamera="",lastCameraArgs=null,actorSurface,foregroundSurface,gateState="normal",debugOutput;let depthZones={behindForegroundAreas:[]};const debug=params.get("sceneDebug")==="1";
...
async function loadDepthZones(){try{const r=await fetch(`${DEPTH_URL}${DEPTH_URL.includes("?")?"&":"?"}t=${Date.now()}`,{cache:"no-store"});if(!r.ok)return;const d=await r.json();if(d?.map!=="star-country-gate-garden"||d?.referenceSize?.width!==REF.w||d?.referenceSize?.height!==REF.h)return;depthZones={behindForegroundAreas:normalizePolys(d.behindForegroundAreas)}}catch(e){console.warn("前後関係データを読み込めませんでした",e)}}
const footIn=(foot,areas)=>areas.some(a=>layout.contains(foot,a));
function drawMaskedActor(ctx,actor,scale,density,draw,opacity=1){const foot={x:actor.x/scale.x,y:actor.y/scale.y},masked=footIn(foot,depthZones.behindForegroundAreas);if(!masked&&opacity===1){draw(ctx);return}actorSurface||=document.createElement("canvas");const left=actor.x-84*scale.x,top=actor.y-110*scale.y,width=168*scale.x,height=142*scale.y,pw=Math.ceil(width*density),ph=Math.ceil(height*density);if(actorSurface.width!==pw||actorSurface.height!==ph){actorSurface.width=pw;actorSurface.height=ph}const paint=actorSurface.getContext("2d");paint.setTransform(1,0,0,1,0,0);paint.clearRect(0,0,pw,ph);paint.setTransform(pw/width,0,0,ph/height,-left*pw/width,-top*ph/height);draw(paint);if(masked){paint.save();paint.globalCompositeOperation="destination-out";if(foregroundSurface)paint.drawImage(foregroundSurface,0,0,REF.w*scale.x,REF.h*scale.y);paint.restore()}ctx.save();ctx.globalAlpha*=Math.max(0,Math.min(1,Number(opacity)||0));ctx.drawImage(actorSurface,0,0,pw,ph,left,top,width,height);ctx.restore()}
function drawDebug(ctx,actors,scale,zoom){if(!debug)return;ctx.save();ctx.scale(scale.x,scale.y);ctx.lineWidth=1/zoom;for(const a of depthZones.behindForegroundAreas){ctx.strokeStyle="#d88cff";layout.trace(ctx,a.points);ctx.stroke()}ctx.restore();debugOutput.textContent="SCENE SIMPLE DEPTH · default=front · purple=behind"}
