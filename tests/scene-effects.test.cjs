...
test('motion uses independent cycles; Preview 44 cloud artwork scrolls left in a buffered track',()=>{
 ...
 assert.doesNotMatch(html,/scene-cloud-far-track/);
 assert.doesNotMatch(html,/scene-cloud-near-track/);
 assert.match(cloudCss,/@keyframes cloud-main-scroll-left[\s\S]*?translate3d\(-33\.333333%, 0, 0\)[\s\S]*?translate3d\(-66\.666667%, 0, 0\)/);
